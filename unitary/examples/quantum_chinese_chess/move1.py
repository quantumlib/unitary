...
def apply_move(str_to_parse: str, board: Board) -> None:
    ...
    # Chances are normal/excluded/capture slide/jump.
    if len(sources) == 1 and len(targets) == 1:
        ...
        if len(quantum_pieces) == 0:
            ...
            if not source.is_entangled() and not target.type_ == Type.EMPTY:
                # TODO(): implement this copy, and check if we could do
                # target = Piece(source)
                board[targets[0]] = Piece(source)
                # TODO(): check if we could do source.clear()
                board[sources[0]].clear()
