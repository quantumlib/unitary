...
class Board:
    ...
    def move_piece(self, from_row, from_col, to_row, to_col):
        ...
        self.board_matrix[to_row][to_col].occupant = Piece(self.board_matrix[from_row][from_col].occupant.color, self.board_matrix[from_row][from_col].occupant.king)
        ...
        if(abs(to_row-from_row) > 1):
            self.remove_piece(int((to_row+from_row)/2), int((to_col+from_col)/2))
        

        # If it is not a king and reaches the end it needs to be kinged
        if(self.board_matrix[to_row][to_col].occupant.king == False and (to_row == 0 or to_row == self.num_rows-1)):
