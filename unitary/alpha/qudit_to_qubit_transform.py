...
def _nearest_power_of_two_ceiling(x: int) -> int:
    exp = 0
    while x:
        x = x >> 1
        exp += 1
...
def qudit_to_qubit_state_impl(
    ...
    padding_amount = _nearest_power_of_two_ceiling(dim_single_qudit) - dim_single_qudit
    ...
    padded_state_tensor = np.pad(
        state_tensor, (0, padding_amount), constant_values=pad_value
    )
    # Return a flattened state vector view of the final tensor.
    return np.ravel(padded_state_tensor)


def qudit_to_qubit_from_map(
    qudit_state_vector: np.ndarray, qubit_to_qudit_index_map: np.ndarray
):
    return [
        qudit_state_vector[qubit_to_qudit_index_map[j] - 1]
        if qubit_to_qudit_index_map[j]
        else 0
        for j in range(len(qubit_to_qudit_index_map))
    ]
...
def qudit_to_qubit_unitary(
    ...
    padded_unitary = qudit_to_qubit_state(
        dim_single_qudit, num_qudits**2, np.ravel(qudit_unitary)
    )
